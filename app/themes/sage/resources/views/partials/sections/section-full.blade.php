@php
  ...
  // Flex State
  $flex_state = get_sub_field('section_builder_flex') === 'yes' ? 'brm-container--flex-center' : null;
...
@endphp
<section class="brm-section brm-section--{{ $i }} brm-section--{{ $section_state }} {{ $background_state }} bg-{{ $background_color_state }} {{ $action_state }}" data-mobile="{{ $bg_mobile }}" data-desktop="{{ $bg_desktop }}">
  @if( $content )
    <div class="brm-container {{ $flex_state }} {{ $text_state }}">
      {!! $content !!}
    </div>
  @endif
</section>
