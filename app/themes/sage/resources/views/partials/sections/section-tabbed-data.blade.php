...
@if( have_rows($rows) )
  <div class="tabs">
    @php
      ...
      $j = 0;
    @endphp
    @if( is_page_template(['views/template-investment-strategies.blade.php']) )
      <h4>FPA Apartment Opportunity Fund <span class="active-tab-caption"></span></h4>
    @endif
    @while( have_rows($rows) ) @php the_row() @endphp
        ...
        <div class="w-full flex flex-row flex-wrap md:flex-no-wrap md:items-start md:justify-center">
          @if( have_rows($data) )
            @while( have_rows($data) ) @php the_row() @endphp
              <div class="flex flex-col flex-wrap mx-1 py-5 px-5 bg-white shadow">
      ...
      @php $i++ @endphp
    @endwhile
    <div class="flex flex-row flex-no-wrap justify-between w-full md:max-w-half mt-10 mx-auto">
