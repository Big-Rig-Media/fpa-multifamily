...
class App extends Controller
{
    ...
    public function location() {
        if ( get_option('location_street') && get_option('location_city') && get_option('location_state') && get_option('location_zipcode') ) {
            $html = '<div itemscope itemtype="http://schema.org/RVPark">
                        ...
                        <p class="mt-3 mb-0"><a itemprop="telephone" href="tel:'.preg_replace('/[^0-9]/', '', self::phone()).'">'.self::phone().'</a></p>
                        <p><a itemprop="email" href="mailto:'.self::email().'">'.self::email().'</a></p>
                        </div>
                     </div>';
    ...
    public function social() {
        ...
        $social_mods = [
            ...
            'campground-reviews' => [
                'svg' => '<svg viewBox="0 0 366 130" xmlns="http://www.w3.org/2000/svg"><g><path d="M0 64.908C0 29.62 32.115 0 70.12 0 90.459 0 109.08 8.41 122 21.573L98.248 41.87c-6.84-8.229-17.103-13.53-28.127-13.53-19.95 0-36.864 16.819-36.864 36.568 0 19.93 16.915 36.752 36.864 36.752 11.212 0 21.287-5.305 28.316-13.53L122 108.242C109.08 121.59 90.458 130 70.12 130 32.116 130 0 100.199 0 64.908M185.612 55.766H244v52.476C230.94 121.953 211.354 130 191.58 130 153.713 130 122 100.195 122 64.908S153.713 0 191.58 0c19.774 0 39.36 8.043 52.42 21.573L220.681 41.87c-6.903-8.413-18.095-13.53-29.101-13.53-19.96 0-36.934 16.819-36.934 36.568 0 19.93 16.974 36.752 36.934 36.752 7.462 0 15.113-2.378 21.454-6.585V79.351h-27.422zM287.35 59.796h24.2c10.818 0 18.515-6.053 18.515-15.136 0-9.085-7.697-15.139-18.514-15.139H287.35zM330.433 130l-27.681-40.683H287.35V130H256V0h58.485c27.681 0 47.85 18.356 47.85 44.66 0 18.92-10.453 33.871-26.768 40.683L366 130z"/></g></svg>',
                'url' => get_option('campground_reviews_url'),
                'title' => 'Campground Reviews'
            ],
            'guest_rated' => [
                'svg' => '<svg viewBox="0 0 25 26" xmlns="http://www.w3.org/2000/svg"><g><circle cx="13.5" cy="15.5" r="3.5"/><path d="M14 11.5a3.5 3.5 0 1 1 3.5 3.5 3.5 3.5 0 0 0-3.5-3.5zM6 26h14v-4.178c-.52-2.101-1.953-3.009-4.301-2.725l-2.493 2.725-2.575-2.725c-2.262-.414-3.806.494-4.631 2.725zM17.5 17.895c.754-.856 1.712-1.813 2.875-2.87 2.547-.198 4.082.759 4.605 2.87.027 1.773.027 3.14 0 4.105H21.5c0-1.634-.5-2.801-1.5-3.5-.087 0-.96-.605-2.5-.605zM4.314 9.54C1.764 8.74 0 7.011 0 5.007 0 2.242 3.358 0 7.5 0 11.642 0 15 2.242 15 5.007s-3.358 5.006-7.5 5.006c-.817 0-1.603-.087-2.34-.248.226.084.34.167.34.248-.205 1.161.591 2.515.774 2.987-1.195-1.095-1.946-2.267-2.251-3.517.102.02.199.038.29.057z"/></g></svg>',
                'url' => get_option('guest_rated_url'),
                'title' => 'Guest Rated'
            ],
        ];
    ...
    public function copyright() {
        ...
        <p class="text-xs font-avenirnextlight">Please see important <a class="text-current no-underline" href="'.get_permalink(1867).'">disclaimers</a>.</p>';
    }
    ...
    public static function capitalization( $object )
    {
        if ( $object ) {
            ...
            if ( $capitalization ) {
                return round(number_format($capitalization));
            }
